...
class DatabaseStorage {
    ...
    async getOrdersBySession(sessionId) {
        return await database_1.db.select().from(schema_1.orders).where((0, drizzle_orm_1.eq)(schema_1.orders.sessionId, sessionId));
    }
    ...
    async getPaymentByReference(reference) {
        const result = await database_1.db.select().from(schema_1.payments).where((0, drizzle_orm_1.eq)(schema_1.payments.reference, reference));
        return result[0];
    }
    async createCryptoTransaction(transaction) {
        const result = await database_1.db.insert(schema_1.cryptoTransactions).values(transaction).returning();
        return result[0];
    }
